"""
This python script can change YOLO's format label back to y1, x1, y2, x2 coordinate by the given text file.
You can run this to convert YOLO's output.
"""
...
train_yolo_label_path = r'.\runs\detect\exp4\labels'
...
output_label_path = r'd:\ML_Final_Project\yolo_code\yolov7\datasets\labels\CNN_labels_005_exp49'
        ...
        with open(f'{label_path}\{name}.txt', 'r') as f:
            with open(f'{output_label_path}\{name}.txt', 'w') as f2:
                for j in f.readlines():
                    ...
                    # y1 x1 y2 x2
                    f2.write(f'{ymin} {xmin} {ymax} {xmax} {conf}\n')
